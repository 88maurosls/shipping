...
def process_shipping_rows(rows, countrycode_dict):
    ...
    for index, row in adjusted_rows.iterrows():
        ...
        if nazione in countrycode_dict:
            ...
            try:
                # Rimuovi spazi bianchi e converti a float
                costo_spedizione = float(row[' COSTI_SPEDIZIONE'].strip())
                ...
                errors.append(f"Valore non valido per COSTI_SPEDIZIONE nella riga {index + 1}: {row[' COSTI_SPEDIZIONE']}")
